...
function checkNoList(list) {
	if (list != null) throw new Error("Unsupported command in list");
}
...
function drawArraysImpl(mode, first, count) {
	...
	glCtx.uniformMatrix4fv(
		mvLocation,
		false,
		modelViewMatrixStack[modelViewMatrixStack.length - 1],
	);
	glCtx.uniformMatrix4fv(
		projLocation,
		false,
		projMatrixStack[projMatrixStack.length - 1],
	);
	assert(first == 0);
...
function pushDrawArraysInList(list, v, mode, first, count) {
	var args = [
		...
		first,
		count,
		captureData(v, vertexData, count),
		captureData(v, colorData, count),
		captureData(v, texCoordData, count),
	];
	list.push({ f: drawArraysInList, a: args });
}
function drawArraysInList(
	mode,
	first,
	count,
	capturedVertexData,
	capturedColorData,
	capturedTexCoordData,
) {
	// Upload vertex data
...
function Java_org_lwjgl_opengl_LinuxContextImplementation_nSwapBuffers() {
	if (verboseLog) console.warn("SwapBuffer");
...
function Java_org_lwjgl_opengl_GL11_nglOrtho(
	lib,
	left,
	right,
	bottom,
...
function Java_org_lwjgl_opengl_GL11_nglTranslatef(lib, x, y, z, funcPtr) {
	...
	var out = glMatrix.mat4.create();
	setCurMatrixTop(
		glMatrix.mat4.translate(out, m, glMatrix.vec3.fromValues(x, y, z)),
	);
}

function Java_org_lwjgl_opengl_GL11_nglViewport(
	lib,
	x,
	y,
	width,
...
function Java_org_lwjgl_opengl_GL11_nglDisable(lib, a, funcPtr) {
	...
	if (a == glCtx.BLEND || a == glCtx.CULL_FACE || a == glCtx.DEPTH_TEST)
		glCtx.disable(a);
	else if (a == 0xde1 /*GL_TEXTURE_2D*/) glCtx.uniform1f(texMaskLocation, 0);
	else if (verboseLog) console.log("glDisable " + a.toString(16));
}
...
function Java_org_lwjgl_opengl_GL11_nglEnable(lib, a, funcPtr) {
	...
	if (a == glCtx.BLEND || a == glCtx.CULL_FACE || a == glCtx.DEPTH_TEST)
		glCtx.enable(a);
	else if (a == 0xde1 /*GL_TEXTURE_2D*/) glCtx.uniform1f(texMaskLocation, 1);
	else if (verboseLog) console.log("glEnable " + a.toString(16));
}
...
function Java_org_lwjgl_opengl_GL11_nglTexParameteri(
	lib,
	target,
	pname,
	param,
...
function Java_org_lwjgl_opengl_GL11_nglTexImage2D(
	lib,
	target,
	level,
	internalFormat,
...
function Java_org_lwjgl_opengl_GL11_nglTexCoordPointer(
	lib,
	size,
	type,
	stride,
...
function Java_org_lwjgl_opengl_GL11_nglColorPointer(
	lib,
	size,
	type,
	stride,
...
function Java_org_lwjgl_opengl_GL11_nglVertexPointer(
	lib,
	size,
	type,
	stride,
...
function Java_org_lwjgl_opengl_GL11_nglDrawArrays(
	lib,
	mode,
	first,
	count,
...
function Java_org_lwjgl_opengl_GL11_nglColor3f() {
	...
	if (verboseLog) console.log("glColor3f");
}
...
function Java_org_lwjgl_opengl_GL11_nglShadeModel() {
	checkNoList(curList);
	if (verboseLog) console.log("glShaderModel");
}
...
function Java_org_lwjgl_opengl_GL11_nglPushMatrix(lib, funcPtr) {
	...
	curMatrixStack.push(
		glMatrix.mat4.clone(curMatrixStack[curMatrixStack.length - 1]),
	);
}
...
function Java_org_lwjgl_opengl_GL11_nglCopyTexSubImage2D(
	lib,
	target,
	level,
	xoffset,
...
function Java_org_lwjgl_opengl_GL11_nglScalef(lib, x, y, z, funcPtr) {
	...
	var out = glMatrix.mat4.create();
	setCurMatrixTop(
		glMatrix.mat4.scale(out, m, glMatrix.vec3.fromValues(x, y, z)),
	);
}

function Java_org_lwjgl_opengl_GL11_nglCallLists(
	lib,
	n,
	type,
	memPtr,
	...
	var v = lib.getJNIDataView();
	var buf = new Int32Array(v.buffer, Number(memPtr), n);
	for (var i = 0; i < n; i++) callList(buf[i]);
}
...
function Java_org_lwjgl_opengl_GL11_nglTexSubImage2D(
	lib,
	target,
	level,
	xoffset,
...
function Java_org_lwjgl_opengl_GL13_nglActiveTexture() {
	checkNoList(curList);
	if (verboseLog) console.log("glActiveTexture");
}

function Java_org_lwjgl_opengl_GL11_nglLightfv() {
	checkNoList(curList);
	if (verboseLog) console.log("glLightfv");
}

function Java_org_lwjgl_opengl_GL11_nglLightModelfv() {
	checkNoList(curList);
	if (verboseLog) console.log("glLightModelfv");
}

function Java_org_lwjgl_opengl_GL11_nglNormalPointer(
	lib,
	type,
	stride,
	memPtr,
...
function Java_org_lwjgl_opengl_GL13_nglMultiTexCoord2f() {
	checkNoList(curList);
	if (verboseLog) console.log("glMultiTexCoord2f");
}

function Java_org_lwjgl_opengl_GL13_nglClientActiveTexture() {
	if (verboseLog) console.log("glClientActiveTexture");
}

function Java_org_lwjgl_opengl_GL11_nglLineWidth() {
	checkNoList(curList);
	if (verboseLog) console.log("glLineWidth");
}

function Java_org_lwjgl_opengl_GL11_nglPolygonOffset() {
	checkNoList(curList);
	if (verboseLog) console.log("glPolygonOffset");
}
...
function Java_org_lwjgl_opengl_GL11_nglEnd(lib, funcPtr) {
	...
	// Upload vertex data
	uploadDataImpl(
		immediateModeData.vertexBuf.subarray(0, immediateModeData.vertexPos),
		vertexBuffer,
		vertexPosition,
		3,
		glCtx.FLOAT,
	...
	// Upload tex coord data
	uploadDataImpl(
		immediateModeData.texCoordBuf.subarray(
			0,
			immediateModeData.texCoordPos,
		),
		texCoordBuffer,
		texCoord,
		2,
...
function Java_org_lwjgl_opengl_LinuxKeyboard_lookupKeysym(
	lib,
	eventPtr,
	index,
) {
...
async function Java_org_lwjgl_opengl_LinuxKeyboard_lookupString(
	lib,
	eventPtr,
	buffer,
) {
